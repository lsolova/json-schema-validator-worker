...
use crate::schema_utils::{convert_schema_to_value, is_id, is_json, is_uri};
...
impl WasmSchemaValidator {
    ...
    pub async fn add_schema(&self, id: String, schema: &str) -> Result<(), String> {
        ...
        let schema_content = convert_schema_to_value(schema)?;
    ...
    pub async fn validate(&self, schema: &str, content: &str) -> Result<bool, String> {
        let json_schema = match schema {
            ...
            s if is_json(s) => convert_schema_to_value(schema)?,
            _ => {
